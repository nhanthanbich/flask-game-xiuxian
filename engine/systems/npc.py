...
class NPCSystem:
    ...
    def get_greeting(self, npc_id: str, relation: dict) -> str:
        ...
        if relation.get("fear", 0) >= 6:
            return f"{npc['name_vn']} can trong lui nua buoc: 'Uy danh cua nguoi khong nho.'"
        if relation.get("trust", 0) >= 6:
            return f"{npc['name_vn']} mim cuoi: 'Ta tin nguoi co the di xa hon.'"
        if relation.get("respect", 0) >= 6:
            return f"{npc['name_vn']} chap tay: 'Thuc luc cua nguoi dang duoc cong nhan.'"
        if npc["personality"] == "friendly":
            return f"{npc['name_vn']}: 'Can gi thi cu hoi ta.'"
        if npc["personality"] == "strict":
            return f"{npc['name_vn']}: 'Giu tam tinh, dung nong voi tien do.'"
        if npc["personality"] == "greedy":
            return f"{npc['name_vn']}: 'Co loi ich thi moi de noi chuyen.'"
        return f"{npc['name_vn']}: 'Nhan qua cua nguoi con chua ro.'"

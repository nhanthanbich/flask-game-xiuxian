...
def default_player(root_id="metal", race_id="human", name="Tan Tu"):
    return {
        ...
        # Demonic Path fields
        "is_demonic": False,
        "demonic_arts_count": 0,
        "demonic_corruption": 0,
        "demonic_realm": None,
        "demonic_breakthroughs": 0,
        # Secret Realm fields
        "secret_realms_cleared": [],
        # Seclusion fields
        "in_seclusion": False,
        "seclusion_location": None,
        "seclusion_duration_remaining": 0,
        "seclusion_start_date": None,
        # Branching Quest fields
        "quest_branch_flags": {},
        "completed_branch_quests": [],
        "quest_consequences": {},
    }
...
def ensure_defaults(state):
    ...
    state["player"].setdefault("root_id", "metal")
    state["player"].setdefault("exp", 0)
    ...
    state["player"].setdefault("birth_year", state["time"].get("year", 1) - state["player"]["age"])
    # Demonic Path defaults
    state["player"].setdefault("is_demonic", False)
    state["player"].setdefault("demonic_arts_count", 0)
    state["player"].setdefault("demonic_corruption", 0)
    state["player"].setdefault("demonic_realm", None)
    state["player"].setdefault("demonic_breakthroughs", 0)
    # Secret Realm defaults
    state["player"].setdefault("secret_realms_cleared", [])
    # Seclusion defaults
    state["player"].setdefault("in_seclusion", False)
    state["player"].setdefault("seclusion_location", None)
    state["player"].setdefault("seclusion_duration_remaining", 0)
    state["player"].setdefault("seclusion_start_date", None)
    # Branching Quest defaults
    state["player"].setdefault("quest_branch_flags", {})
    state["player"].setdefault("completed_branch_quests", [])
    state["player"].setdefault("quest_consequences", {})
...
def set_time(state, time):
    ...
    # Update player age based on years passed
    player = state["player"]
    birth_year = player.get("birth_year", time.year - player.get("age", 15))
    player["birth_year"] = birth_year
    player["age"] = time.year - birth_year
...
def render_page(tab, body):
    state = get_state()
    player = state["player"]
    player_realm = player.get("realm_id", "mortal")
    player_root = player.get("root_id", "metal")
    realm = cult.realms.get(player_realm, cult.realms.get("mortal", {"name_vn": "Không rõ"}))
    root = cult.roots.get(player_root, {"name_vn": "Không rõ"})
    race = races.get(player.get("race_id", "human"))
    nxt = cult.next_realm(player_realm)

    # Tu vi progress is EXP only. Pressure is a separate flavor/risk layer.
    if not nxt:
        exp_progress = 100.0
        pressure_required = 0
        pressure_progress = 100.0
        progress = 100.0
    else:
        exp_current = player.get("exp", 0)
        exp_required_value = max(1, int(nxt["exp_required"]))
        exp_progress = min(100.0, round(exp_current / exp_required_value * 100, 2))
        pressure_required = cult.breakthrough_pressure_requirement(player_realm)
        pressure_current = player.get("cultivation_pressure", 0)
        pressure_progress = min(100.0, round(pressure_current / max(1, pressure_required) * 100, 2)) if pressure_required > 0 else 100.0
    ...
    context = {
        ...
        "progress": progress,
        "progress_max": 100,
        "exp_progress": exp_progress if nxt else 100,
        "pressure_progress": pressure_progress if nxt else 100,
        "exp_required": int(nxt["exp_required"]) if nxt else 0,
        "exp_current": player.get("exp", 0),
        "pressure_required": pressure_required if nxt else 0,
        "pressure_current": player.get("cultivation_pressure", 0),
        "sect_name": sect_name,
        "tabs": tabs,
        "tab": tab,
...
@app.post("/cultivate/<months>")
def cultivate(months):
    state = get_state()
    player = state["player"]
    time = current_time(state)

    # Convert to float to support fractional months (e.g., 0.1 for 3 days)
    try:
        months = float(months)
    ...
    if pressure_result.get("event") == "táu_hoả_nhập_ma":
        ...
        # Add realm-appropriate flavor text
        flavor_text = cult.get_cultivation_flavor(player["realm_id"], months)
        log(state, flavor_text)
        log(state, f"Nhận {adjusted_gain} exp.")
...
def breakthrough():
    ...
    pressure_required = cult.breakthrough_pressure_requirement(player.get("realm_id", "mortal"))
    if pressure < pressure_required:
        log(state, f"Áp lực đột phá chưa đủ: cần {pressure_required}%, hiện tại {pressure}%.")
        save_state(state)
        return redirect(url_for("game", tab="cultivate"))
...
def breakthrough_seclusion():
    ...
    pressure_required = cult.breakthrough_pressure_requirement(player.get("realm_id", "mortal"))
    if pressure < pressure_required:
        log(state, f"Áp lực đột phá chưa đủ để bế quan: cần {pressure_required}%, hiện tại {pressure}%.")
        save_state(state)
        return redirect(url_for("game", tab="cultivate"))
...
def explore_secret(realm_id):
    ...
    difficulty = float(secret.get("difficulty", 1))
    danger = min(0.95, max(0.02, difficulty / 5.0))
    state["secret_state"] = {
        "id": realm_id,
        "progress": 0,
        "danger": danger,
        "logs": [f"Ngươi bước vào {secret.get('name_vn', secret.get('id', 'Bí cảnh'))}."]
    }
    save_state(state)
    return redirect(url_for("game", tab="secret"))
...
def secret_action(action):
    ...
    secret = world.secret_realms.get(sstate["id"])
    if not secret:
    ...
    if action == "observe":
        ...
        log(state, f"Ngươi rời {secret.get('name_vn', secret.get('id', 'Bí cảnh'))} trước khi lún sâu.")
        state["secret_state"] = None
    elif action == "advance":
        import random
        sstate["progress"] += 1
        if random.random() < sstate["danger"]:
            time = current_time(state)
            time.advance(max(1, int(secret.get("time_cost_days", 3)) // 2))
            tick_time(state, time)
            set_time(state, time)
            log(state, f"Gặp biến cố trong {secret.get('name_vn', secret.get('id', 'Bí cảnh'))}, thám hiểm thất bại.")
            state["secret_state"] = None
        elif sstate["progress"] >= 3:
            time = current_time(state)
            time.advance(int(secret.get("time_cost_days", 3)))
            tick_time(state, time)
            set_time(state, time)
            state["player"]["exp"] += int(secret.get("exp_reward", 0))
            loot_items = [item_id for item_id in secret.get("loot_items", "").split("+") if item_id]
            if loot_items:
                inv = state["player"].setdefault("inventory", {})
                for reward_item in loot_items:
                    inv[reward_item] = inv.get(reward_item, 0) + 1
            log(state, f"Khám phá trọn vẹn {secret.get('name_vn', secret.get('id', 'Bí cảnh'))}, nhận {secret.get('exp_reward', 0)} exp.")
            state["secret_state"] = None
        else:
...
def combat_use(technique_id):
    ...
    if technique_id not in tech.techniques:
        ...
        cstate["logs"].append("Không đủ MP, người điều tưởng để hồi linh lực.")
    else:
...
def combat_recover():
    ...
    if cstate:
        ...
        cstate["logs"].append("Người lui nửa bước, điều tưởng hồi MP.")
...
def combat_flee():
    ...
    if cstate:
        ...
        tick_time(state, time)
        set_time(state, time)
        log(state, f"Người rút khỏi trận với {cstate['e_state']['name']}.")
        state["combat_state"] = None
    save_state(state)
    return redirect(url_for("game", tab="combat"))
...
def view_cultivate():
    ...
    # Get dynamic seclusion durations based on realm
    seclusion_options = cult.get_seclusion_durations(player["realm_id"])
    buttons = "".join(
        f'<div class="cultivate-option">'
        f'<div class="option-header">'
        f'<span class="option-icon">🧘</span>'
        f'<span class="option-time">{label}</span>'
        f'</div>'
        f'<div class="option-gain">+{cult.calc_player_exp(months, player, races)} exp</div>'
        f'<form method="post" action="{url_for("cultivate", months=months)}">'
        f'<button class="btn btn-cultivate">Bế Quan</button>'
        f'</form></div>'
        for months, label in seclusion_options
    )
...
def view_techniques():
    ...
    for technique in tech.get_learnable(player["realm_id"]):
        cards.append(
            ...
            f"<span class='technique-element'>{technique.get('element', 'Vô') if technique.get('element') and technique.get('element') != 'None' else 'Vô'}</span>"
            f"</div>"
...
def view_sect():
    ...
    return f"<h2>{sect['name_vn']}</h2><p>Cấp bậc {state['world_state'].get('player_rank', 0)} | Trận thắng chỉ nộp: {state['world_state'].get('combat_wins', 0)}</p>{buttons}<h3>Nhiệm vụ môn phái</h3><div class='grid'>{quest_cards}</div><h3>NPC</h3><ul>{npcs}</ul>"
...
def view_secret():
    ...
    if sstate:
        ...
        if not secret:
            state["secret_state"] = None
            return "<h2>Bí cảnh</h2><p>Bí cảnh không tồn tại.</p>"

        logs = "".join(f"<div>{line}</div>" for line in sstate.get("logs", []))
        return (
            f"<h2>{secret.get('name_vn', 'Bí cảnh không rõ')}</h2>"
            f"<p>Tiến độ {sstate['progress']}/3 | Rủi ro hiện tại {int(sstate.get('danger', 0) * 100)}%</p>"
            f"<p class='muted'>{secret.get('description', '')}</p>"
            ...
            f"<form method='post' action='{url_for('secret_action', action='retreat')}'><button>Rút lui</button></form>"
            f"<h3>Diễn biến</h3><div class='log'>{logs}</div>"
        )
    cards = []
    for secret in world.get_available_secret_realms(state["player"]):
        secret_id = secret.get('id', '')
        time_cost = secret.get('time_cost_days', '?')
        difficulty = secret.get('difficulty', '')
        difficulty_label = f"Độ khó {difficulty}" if difficulty else ""
        cards.append(
            f"<div class='card'><h3>{secret.get('name_vn', secret_id or 'Bí cảnh')}</h3>"
            f"<p>Thời gian {time_cost} ngày{(' | ' + difficulty_label) if difficulty_label else ''}</p>"
            f"<p class='muted'>{secret.get('description', '')}</p><form method='post' action='{url_for('explore_secret', realm_id=secret_id)}'><button>Thám hiểm</button></form></div>"
        )
...
def view_combat():
    ...
    if cstate:
        ...
        return (
            ...
            f"<form method='post' action='{url_for('combat_recover')}'><button>Äiá»u tương há»“i MP</button></form>"
    ...
    for enemy in Loader.load(ENEMIES_PATH):
        ...
        enemy_element = enemy['element'] if enemy['element'] and enemy['element'] != 'None' else 'Vô'
        cards.append(
            f"<div class='card'><h3>{enemy['name_vn']}</h3><p>{enemy_realm_name} | {enemy_element} | HP {enemy['hp']}</p>"

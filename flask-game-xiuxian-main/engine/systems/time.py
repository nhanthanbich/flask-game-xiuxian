...
class TimeSystem:
    ...
    def get_years_passed(self, start_year: int) -> int:
        """Calculate years passed from a starting year."""
        return self.year - start_year

    def advance_months(self, months: float = 1):
        """Tiện ích: tăng N tháng (= N × 30 ngày). Hỗ trợ số thập phân cho ngày."""
        days = int(months * self.DAYS_PER_MONTH)
        if days < 1:

...
from engine.systems.secret_realm import SecretRealmSystem
...
class WorldSystem:
    def __init__(self, settings: dict | None = None):
        ...
        self.cult = CultivationSystem(settings)
        self.secret_realm_system = SecretRealmSystem()
    ...
    def get_available_secret_realms(self, player: dict) -> list[dict]:
        available = []
        for realm_id, realm in self.secret_realms.items():
            can_enter, _ = self.secret_realm_system.check_requirement(player or {}, realm_id)
            if can_enter:
                available.append(realm)
        return available
    ...
    def get_available_sects(self, player: dict) -> list[dict]:
        ...
        for sect in self.sects.values():
            ...
            entry_condition = sect.get("entry_condition", "").strip()

            if entry_condition == "linh_can_match":
                # Check if player's root matches sect element
                player_root = self.cult.roots.get(player.get("root_id", ""))
                if player_root:
                    root_element = player_root.get("element", "")
                    sect_element = sect.get("elements") or sect.get("element", "")

                    # Check match - sect can have multiple elements separated by pipe
                    if sect_element:
                        allowed_elements = [e.strip() for e in sect_element.split("|")]
                        if root_element not in allowed_elements and root_element != "Ngũ Hành":
                            # Ngũ Hành root can join any sect
                            continue

            elif entry_condition == "quest_and_realm":
                # This requires a specific quest - for now skip this sect
                # unless player has completed the quest
                continue

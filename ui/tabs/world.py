...
class WorldTab:
    ...
    def _show_sects(self, world_state: dict):
        ...
        for sect in self.world.sects.values():
            ...
            all_elements = sect.get('elements', 'Vô').replace('|', ', ')
            Renderer.line(
                f"{sect['name_vn']:16} | {all_elements:16} | {sect['faction']:5} | "
    ...
    def _join(self, player: dict, world_state: dict) -> bool:
        ...
        options = [
            f"{s['name_vn']:16} | {s.get('elements', 'Vô').replace('|', ', '):16} | yêu cầu {self.cult.realm_display(s.get('min_realm', 'mortal')).split('(')[0].strip()}"
            for s in available
        ] + ["Hủy"]
        choice = Renderer.menu(options)
